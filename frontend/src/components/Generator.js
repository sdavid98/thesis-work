...
const generator = (structureData, contents, canvasStyle, rowStyles) => {
    ...
    const getHeightForSpacer = (conId) => parseInt(contents.find(con => con.id === conId).rootElementStyle.height);
            ...
            col.rows.map(colRow => {
                return rows.filter(row => row.id === colRow).map((row, index) => {
                    ...
                    if (row.columns && row.columns.length > 0) {
                        result += `<td width='${col.width}' valign='top'>`;
                        ...
                        if (content.type === 'divider') {
                            result += Spacer(content, col.width, getHeightForSpacer(row.content));
                        }
